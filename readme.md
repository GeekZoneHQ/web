...
> All commands in this section can be run either in docker-compose or in the virtual environment.
...
#### 1. Docker-compose
To test any change in the frontend code run the following commands, depending on whether you need to install or run tailwind, or to generate a production build of `styles.css`:
```sh
docker-compose run web python3 manage.py tailwind install
```
or 
```sh
docker-compose run web python3 manage.py tailwind start
```
or 
```sh
